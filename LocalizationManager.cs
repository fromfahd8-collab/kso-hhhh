...
namespace KSO.Modules
{
    public class LocalizationData
    {
        ...
        public string BrowserTab { get; set; } = "المتصفح"; // ضفناه عشان تبويب WebView2
    ...
    public class LocalizationManager
    {
        ...
        private Dictionary<string, LocalizationData> _languages;
        ...
        public LocalizationManager(string langPath)
        {
            _langPath = langPath;
            LoadLanguages();
            SetLanguage(ConfigManagerStatic.LoadLangCode(_langPath)); // يقرا اخر لغة من config
        }

        private void LoadLanguages()
        {
            EnsureLangExists(); // نفك lang.json من الـ exe

            if (File.Exists(_langPath))
            {
                try { _languages = JsonConvert.DeserializeObject<Dictionary<string, LocalizationData>>(File.ReadAllText(_langPath)); }
        ...
        public void ToggleLanguage()
        {
            ...
            ConfigManagerStatic.SaveLangCode(_langPath, _currentCode); // نحفظ الاختيار
        }
    ...
    // كلاس صغير عشان نحفظ اللغة في config
    internal static class ConfigManagerStatic
    {
        public static string LoadLangCode(string langPath)
        {
            string configPath = Path.Combine(Path.GetDirectoryName(langPath), "config.json");
        ...
        public static void SaveLangCode(string langPath, string code)
        {
            string configPath = Path.Combine(Path.GetDirectoryName(langPath), "config.json");
            if(File.Exists(configPath))
                try { var cfg = JsonConvert.DeserializeObject<ConfigData>(File.ReadAllText(configPath)); cfg.Language = code; File.WriteAllText(configPath, JsonConvert.SerializeObject(cfg, Formatting.Indented)); } catch{}
        }
    }
}
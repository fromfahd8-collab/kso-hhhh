...
namespace KSO.Modules
{
    public class DownloadItem : INotifyPropertyChanged
    {
        ...
        private readonly ConfigData _config; // ضفناه
        ...
        public bool Compressed { get => _compressed; set { _compressed = value; OnPropertyChanged(); } }
        ...
        // عدلنا الكونستركتور عشان ياخد Config
        public DownloadItem(string url, int threads, HttpClient client, ConfigData config)
        {
            ...
            CancelCommand = new RelayCommand(() => Cancel(), () => Status!= "Completed" && Status!= "Cancelled");
        }
        ...
        public async Task StartDownloadAsync()
        {
            ...
            try
            {
                ...
                foreach (var part in _parts) // استعادة
                {
                ...
                if (!_isCancelled && _parts.All(p => p.Completed))
                {
                    ...
                    if (_config.AutoCompress && FfmpegHelper.CheckFfmpeg())
                        _ = AutoCompressAsync();
                }
        ...
        private async Task AutoCompressAsync()
        {
            try
            {
                Status = "Compressing...";
                ...
                string outputFile = Path.ChangeExtension(inputFile, "_KSO_Auto" + ext);
                ...
                bool success = FfmpegHelper.CompressVideo(inputFile, outputFile, crf, preset);
                if (success)
                {
                    Compressed = true;
                    FileName = Path.GetFileName(outputFile);
                    Status = "Compressed";
                    if (_config.DeleteOriginal) File.Delete(inputFile);
                }
